/*
 * Cortex-A57MPCore
 *   for now most a copy of (there must be a better way to do this):
...
{
    A57MPPrivState *s = FROM_SYSBUS(A57MPPrivState, dev);
    SysBusDevice *busdev;
    ...
    qdev_init_gpio_in(&s->busdev.qdev, a57mp_priv_set_irq, s->num_irq - 32);
...
static Property a57mp_priv_properties[] = {
     ...
     */
    DEFINE_PROP_UINT32("num-irq", A57MPPrivState, num_irq, 96),
